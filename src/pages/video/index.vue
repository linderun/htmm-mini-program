<template>
	<div>
		<div v-show="!isMore && videolist.length == 0" class="nodata"><image src="/static/images/nodata.jpg" mode="aspectFit" /></div>
		<video
			id="prew_video"
			autoplay="true"
			direction="0"
						...
						<div class="imgbox clearfix ">
							<span v-for="(image, subitemIndex) in item.imglist" :key="subitemIndex">
								<span class="previewimg" >
									<image :src="image" @click="previewImage(item.imglist, image)" mode="aspectFill" />
								</span>
							
							</span>
							 <span v-if="item.video_url" class="video" @click="playvideo(image.src, item.thumbImg[subitemIndex])">
								<!-- <image :src="item.thumbImg[subitemIndex]" mode="aspectFill" /> -->
